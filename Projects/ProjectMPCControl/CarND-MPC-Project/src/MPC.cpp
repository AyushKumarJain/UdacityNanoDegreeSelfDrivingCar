...
size_t x_i     = 0; // 0   - 11  (N Values)
size_t y_i     = x_i + N; // 12     - 23 (N Values)
size_t psi_i   = y_i + N; // 24     - 35 (N Values)
size_t v_i     = psi_i + N; // 36   - 47 (N Values)
size_t cte_i   = v_i + N; // 48     - 59 (N Values)
size_t epsi_i  = cte_i + N; // 60   - 71 (N Values)
size_t delta_i = epsi_i + N; // 72  - 82 (Only N-1 values)
size_t a_i     = delta_i + N - 1; // 83 (Only N-1 Values)
 
 // Class FG_eval, a class with declaration and its body
 // with public access specifier to access functions outside the class
 // We will use objects to access functions of the class.   
class FG_eval {
public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // cost for the difference between sequential actuations
    for (int i = 0; i < N - 2; i++){
...
Solution MPC::Solve(const VectorXd x0, const VectorXd coeffs) {
   ...
   */
  size_t n_constraints = 6 * N; // 6 equations 
