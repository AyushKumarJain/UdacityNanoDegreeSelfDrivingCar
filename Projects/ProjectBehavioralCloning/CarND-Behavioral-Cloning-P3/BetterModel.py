    ...
    while 1: 
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for batch_sample in batch_samples:
                ...
                # image_bright = color.bgr2hsv(image)
                # image_bright[:, :, 2] *= .5 + .4 * np.random.uniform()
                # image_bright = img_as_ubyte(color.hsv2bgr(image_bright))
...
def Model2():
    ...
    model.add(Convolution2D(48,(5,5), strides
=(2,2), activation='relu'))
...
reducelearnrate = ReduceLROnPlateau(monitor = 'val_loss', factor = 0.2, patience = 3, verbose = 1, min_delta = 0.0001)

callbacks = [earlystop, checkpoint, reducelearnrate]

model.compile(loss='mse', optimizer='adam')
history_object = model.fit_generator(train_generator, samples_per_epoch= len(train_samples), validation_data=validation_generator, nb_val_samples=len(validation_samples), callbacks=callbacks, nb_epoch=3, verbose=1)
