...
float motion_model(float pseudo_position, float movement, vector<float> priors,
  ...
  for (float j=0; j< map_size; ++j) {
    ...
    // The normpdf function needs distance from start position to end position,
    // movement_per_unit_timestep and control standard deviation
    float transition_prob = Helpers::normpdf(distance_ij, movement, 
                                             control_stdev);
    // estimate probability for the motion model, this is our prior
    // Below we get total transition probablity to reach the pseudo_position from different places of the map
